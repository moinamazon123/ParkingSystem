...
<body class="">
  ...
  <div class="wrapper ">
   ...
   rs = null;
             ...
             rs = st.executeQuery("SELECT * from parking.booking bk inner join parking.user usr on usr.user_id=bk.userId where bk.building_no='"+request.getParameter("buildNo")+"' and bk.floor_no="+request.getParameter("floorNo"));
      
      %>
                    
      <div class="content">
          <table>
              <tr><th>Building No</th><th>Floor Number</th><th>Parking Number</th><th>Check In Date</th><th>Check Out Date</th><th>Occupied By</th></tr>
    
              <% while(rs.next()) { %>
              <tr><td><%=rs.getInt("building_no")%></td><td><%=rs.getInt("floor_no")%></td><td><%=rs.getInt("parking_no")%></td><td><%=rs.getString("checkinDate")%></td><td style="background-color:Cyan"><%=rs.getString("checkoutDate")%></font></td><td><%=rs.getString("username")%></td></tr>
